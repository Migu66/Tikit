  ...
  try {
    ...
    switch (periodType) {
      case 'allTime':
        ...
        startDate = firstTicket ? new Date(firstTicket.purchaseDate) : startOfMonth(now);
    ...
    const averagePerTicket = periodTickets.length > 0 
      ? totalSpent / periodTickets.length 
      : 0;
    ...
    const byCategory = Object.entries(categoryData).map(([category, amount]) => ({
      category,
    ...
    const monthsToShow = periodType === 'allTime' || periodType === 'currentYear' ? 12 : 6;
    const monthlyTrends = [];
    ...
    storeTickets.forEach(ticket => {
      ...
      if (ticket.storeName.length < store.displayName.length && 
